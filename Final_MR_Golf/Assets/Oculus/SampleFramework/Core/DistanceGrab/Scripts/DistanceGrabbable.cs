...
namespace OculusSampleFramework
{
    public class DistanceGrabbable : OVRGrabbable
    {
        ...
        private void Update()
        {
            // if time < 1 second, set the position of the obj to the position of the hand
            if (Time.time < 2)
            {
                transform.position = Hand.transform.position;
            }

            // if it is grabbed, set the gravity to true
            if (isGrabbed)
            {
                Rigidbody rb = GetComponent<Rigidbody>();
                rb.useGravity = true;
            }
            else // if it is not grabbed, set the gravity to false
            {
                Rigidbody rb = GetComponent<Rigidbody>();
                rb.useGravity = false;
            }
